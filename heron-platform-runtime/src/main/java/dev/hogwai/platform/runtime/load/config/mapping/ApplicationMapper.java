...
final class ApplicationMapper {
    ...
    private static final Set<String> SPEC_FIELDS = Set.of("capabilities", "entrypoints");
    ...
    static ApplicationConfig map(JsonNode root, List<Diagnostic> diagnostics) {
        ...
        List<CapabilityConfig> capabilities = mapSpec(spec, diagnostics);
        List<EntrypointConfig> entrypoints = spec != null && spec.isObject()
                ? EntrypointMapper.mapEntrypoints(spec.get("entrypoints"), "/spec/entrypoints", diagnostics)
                : List.of();

        if (!diagnostics.isEmpty()) {
            return null;
        }
        return new ApplicationConfig(apiVersion, kind, name, capabilities, entrypoints);
    }
