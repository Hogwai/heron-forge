...
final class PullInvoker {

    private final Clock clock;

    PullInvoker(Clock clock) {
    ...
    MaterializedDataSet invokeTarget(RuntimeSnapshot snapshot, String targetId, ExecutionContext context) {
    ...
    private static final class ProviderExecutor {
        private MaterializedDataSet execute(RuntimeSnapshot snapshot, CapabilityNode node,
            ...
            CapabilityInstance instance = snapshot.instance(node.id()).orElseThrow(ProviderExecutor::configurationFailure);
            try {
