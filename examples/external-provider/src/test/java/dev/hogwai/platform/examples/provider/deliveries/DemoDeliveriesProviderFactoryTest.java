...
class DemoDeliveriesProviderFactoryTest {
    ...
    void readsDeliveriesThroughDataAccessWithStableQueryContextAndOrder() {
        ...
        var first = DeliveriesQuery.read(access, queryContext);
        var second = DeliveriesQuery.read(access, queryContext);
        ...
        assertThat(first.records()).extracting(record -> record.value(new FieldId("deliveredQuantity")))
